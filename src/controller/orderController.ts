...
export const createOrder = async (request: Request, response: Response) => {
  try {
    const { customerName, order_type, order_date, orderDetail } = request.body;
    ...
    for (let index = 0; index < orderDetail.length; index++) {
      const { coffee_id, quantity, price } = orderDetail[index];
...
export const getAllOrder = async (request: Request, response: Response) => {
    try {
        ...
        const allOrder = await prisma.orderList.findMany({
            ...
            include: { orderDetail: {include: {coffee: true}} } 
        })
